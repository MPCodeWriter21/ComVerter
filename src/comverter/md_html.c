...
int is_html_tag(const char *text, size_t len, size_t *consumed) {
    ...
    {
        static const char *disallowed[] = {"title",    "textarea",  "style",
                                           "xmp",      "pre",       "script",
                                           "iframe",   "noembed",   "noframes",
                                           "noscript", "plaintext", NULL};
        size_t nl = tag_end - 1;
        for (int di = 0; disallowed[di]; di++) {
            ...
            if (nl == dl) {
                ...
                for (size_t ci = 0; ci < dl; ci++)
                    if (tolower((unsigned char)text[1 + ci]) != disallowed[di][ci]) {
                        m = 0;
                        break;
                    }
                if (m) return 0;
    ...
    /* Parse attributes */
    int value_parsed =
        0; /* 1 if previous iteration consumed a value (ws required before next attr) */
    while (j < len && text[j] != '>' && text[j] != '/') {
        ...
        j++;
        while (j < len && (isalnum((unsigned char)text[j]) || text[j] == ':' ||
...
int is_html_block_tag(const char *tag, size_t remaining) {
    ...
    while (name_end < remaining &&
           (isalnum((unsigned char)tag[name_end]) || tag[name_end] == '-'))
        name_end++;
...
int is_html_block_start(const char *line, size_t len) {
       ...
       followed only by whitespace or end of line */
    {
        ...
        if (is_html_tag(rest, rest_len, &tag_consumed) && tag_consumed > 0) {
            ...
            while (after < rest_len && (rest[after] == ' ' || rest[after] == '\t'))
                after++;
            if (after == rest_len) {
                ...
                while (tn_len < rest_len - (size_t)(tn - rest) &&
                       (isalnum((unsigned char)tn[tn_len]) || tn[tn_len] == '-'))
                    tn_len++;
                int is_excluded = (tn_len == 6 && ((tn[0] == 's' || tn[0] == 'S') &&
                                                   (tn[1] == 'c' || tn[1] == 'C') &&
                                                   (tn[2] == 'r' || tn[2] == 'R') &&
                                                   (tn[3] == 'i' || tn[3] == 'I') &&
                                                   (tn[4] == 'p' || tn[4] == 'P') &&
                                                   (tn[5] == 't' || tn[5] == 'T'))) ||
                                  (tn_len == 5 && ((tn[0] == 's' || tn[0] == 'S') &&
                                                   (tn[1] == 't' || tn[1] == 'T') &&
                                                   (tn[2] == 'y' || tn[2] == 'Y') &&
                                                   (tn[3] == 'l' || tn[3] == 'L') &&
                                                   (tn[4] == 'e' || tn[4] == 'E'))) ||
                                  (tn_len == 3 && ((tn[0] == 'p' || tn[0] == 'P') &&
                                                   (tn[1] == 'r' || tn[1] == 'R') &&
                                                   (tn[2] == 'e' || tn[2] == 'E'))) ||
                                  (tn_len == 8 && ((tn[0] == 't' || tn[0] == 'T') &&
                                                   (tn[1] == 'e' || tn[1] == 'E') &&
                                                   (tn[2] == 'x' || tn[2] == 'X') &&
                                                   (tn[3] == 't' || tn[3] == 'T') &&
                                                   (tn[4] == 'a' || tn[4] == 'A') &&
...
static int is_disallowed_tag_name(const char *text, size_t len) {
    ...
    for (int di = 0; disallowed[di]; di++) {
        ...
        if (len == dl) {
            int m = 1;
            for (size_t ci = 0; ci < dl; ci++)
                if (tolower((unsigned char)text[ci]) != disallowed[di][ci]) {
                    m = 0;
                    break;
                }
            if (m) return 1;
...
void sb_append_with_disallowed_escaped(
    StringBuilder *sb, const char *text, size_t len
) {
    size_t i = 0;
    while (i < len) {
        if (text[i] == '<') {
            ...
            while (start < len &&
                   (isalnum((unsigned char)text[start]) || text[start] == '-'))
                start++;
